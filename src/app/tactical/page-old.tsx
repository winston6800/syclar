...
export default function TacticalPage() {
  const [tasks, setTasks] = useState<Task[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("tacticalTasks");
      if (saved) {
        try {
  ...
  // If a stone is marked inProgress in localStorage but we don't actually have
  // a running focus session or a matching task, clear the stale marker so the
  // UI doesn't block starting new stones after a reload.
  useEffect(() => {
    try {
      const saved = localStorage.getItem("strategyStones");
      if (saved) {
        const allStones: Stone[] = JSON.parse(saved);
        const anyActive = allStones.some((s) => s.inProgress);
        if (anyActive && !focusSession.isRunning) {
          // No active focus session — clear stale inProgress flags
          const cleared = allStones.map((s) => ({ ...s, inProgress: false }));
          localStorage.setItem("strategyStones", JSON.stringify(cleared));
          // Update local state to match
          const incomplete = cleared.filter((s) => !s.completed);
          setAvailableStones(incomplete);
        }
      }
    } catch (e) {
      console.error("Failed to clean stale inProgress flags:", e);
    }
  }, [focusSession.isRunning, tasks]);

  // Save tasks to localStorage whenever they change
  useEffect(() => {
    try {
      localStorage.setItem("tacticalTasks", JSON.stringify(tasks));
    } catch (e) {
      console.error("Failed to save tasks to localStorage:", e);
    }
  }, [tasks]);

  // Pending stone logic disabled (AI auto-create feature removed)
  // User must now create tasks manually via the "Add a task" input
  ...
  const stopFocus = () => {
    ...
    // Clear inProgress marker for the corresponding stone (if any)
    try {
      ...
      if (task) {
        ...
        if (saved) {
          const allStones: Stone[] = JSON.parse(saved);
          const updated = allStones.map((s) =>
            s.name === task.label ? { ...s, inProgress: false } : s,
          );
          localStorage.setItem("strategyStones", JSON.stringify(updated));
        }
      }
    } catch (e) {
      console.error("Failed to clear stone inProgress:", e);
    }
  ...
  const toggleTaskComplete = (taskId: string) => {
    setTasks((prev) => {
      ...
      // Compute timeSaved before toggling completion
      let timeSaved = 0;
      if (task?.estimatedMinutes) {
        timeSaved = Math.max(0, task.estimatedMinutes - (task.timeSpent || 0));
      }
      
      const updated = prev.map((t) =>
        t.id === taskId ? { ...t, completed: !t.completed, timeSaved } : t,
      );
      ...
      if (nowCompleted && !wasCompleted) {
        ...
        // Compute time saved for stats aggregation
        const taskToComplete = task;
        let timeSaved = 0;
        if (taskToComplete?.estimatedMinutes) {
          timeSaved = Math.max(0, taskToComplete.estimatedMinutes - (taskToComplete.timeSpent || 0));
        }
        
        const updatedStats = recordCycle({
          estimatedMinutes: taskToComplete?.estimatedMinutes || 0,
          actualMinutes: taskToComplete?.timeSpent || 0,
          timeSaved,
        });
        fetch("/api/stats/record", {
          method: "POST",
  ...
  // AI breakdown function removed (feature disabled)
  ...
  // handleBreakDown removed (AI disabled)
